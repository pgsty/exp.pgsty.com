...
如果完全不指定 URL，`pg_exporter` 会回退到本地优先的默认连接串 `postgresql:///?sslmode=disable`，适合与 PostgreSQL 同机部署的场景。完整的 URL 来源优先级（`--url` > `PG_EXPORTER_URL` > `PGURL` > `PG_EXPORTER_URL_FILE` > 默认值）见 [部署指南](/zh/deploy/)。
